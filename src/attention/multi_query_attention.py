...
This reduces memory bandwidth requirements and speeds up inference.
...
class MultiQueryAttention(nn.Module):
    ...
    In MQA, queries have multiple heads but keys and values share a single head.
    This significantly reduces the KV cache size during inference.
    ...
    def __init__(
        ...
        self.k_proj = nn.Linear(d_model, self.d_k, bias=qkv_bias)
        self.v_proj = nn.Linear(d_model, self.d_k, bias=qkv_bias)
    ...
    def forward(
        ...
        K = self.k_proj(key)    # [batch, key_len, d_k]
        V = self.v_proj(value)  # [batch, key_len, d_k]

        # Expand K and V to match number of query heads
        K = K.unsqueeze(1).expand(batch_size, self.num_heads, key_len, self.d_k)
        V = V.unsqueeze(1).expand(batch_size, self.num_heads, key_len, self.d_k)
    ...
    def extra_repr(self) -> str:
        return f'd_model={self.d_model}, num_heads={self.num_heads}, d_k={self.d_k} (MQA)'
